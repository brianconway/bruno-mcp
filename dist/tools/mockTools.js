...
export function registerMockTools(server, deps) {
    server.registerTool('create_mock_server', {
        title: 'Create Mock Server Config',
        description: "Define a set of canned HTTP routes for a collection, persisted as a JSON config. Bruno's own mock server is a desktop-app-only beta feature with no CLI/scripting hook — this is a fully scriptable equivalent that doesn't need the GUI.",
        inputSchema: {
    ...
    server.registerTool('start_mock_server', {
    ...
    server.registerTool('stop_mock_server', {
    ...
    server.registerTool('list_mock_servers', {

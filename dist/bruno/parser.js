 ...
 */
import { BruFileError, } from './types.js';
 ...
 */
export function parseBruFile(content) {
 ...
 * it — see folder.ts.
 */
export function splitTopLevelBlocks(rawLines) {
    const blocks = [];
    let i = 0;
 ...
 */
function dedentLine(line, indentSize = DEFAULT_INDENT_SIZE) {
    if (line.trim() === '') {
        return line;
    }
    if (line.startsWith('\t')) {
 ...
 * interpret.
 */
export function parseKeyValueBlock(rawLines, indentSize = DEFAULT_INDENT_SIZE) {
...
function parseStringMap(rawLines) {
 ...
 */
export function parseBlobText(rawLines, indentSize = DEFAULT_INDENT_SIZE) {
 ...
 */
function parseBlobLines(rawLines, indentSize = DEFAULT_INDENT_SIZE) {
...
function parseHttpBlock(methodWord, lines) {
    const map = parseKeyValueBlock(lines);
    const url = map.get('url');
    const bodyToken = map.get('body');
    const authToken = map.get('auth');
    return {
        method: methodWord.toUpperCase(),
        url: typeof url === 'string' ? url : '',
        body: (typeof bodyToken === 'string' ? bodyToken : 'none'),
        auth: (typeof authToken === 'string' ? authToken : 'none'),
    };
}
function asString(map, key) {
    const value = map.get(key);
    if (value === undefined)
        return undefined;
    return typeof value === 'string' ? value : String(value);
}
function asGrantType(value) {
    return value === 'authorization_code' || value === 'password' ? value : 'client_credentials';
}
function asApiKeyIn(value) {
    return value === 'query' ? 'query' : 'header';
}
function parseAuthBlock(subtype, lines) {
    const map = parseKeyValueBlock(lines);
    const auth = { type: subtype };
    switch (subtype) {
        case 'bearer':
...
function parseBodyBlock(subtype, lines) {

...
import { CreateRequestInput, HttpMethod, AuthType, BodyType } from '../bruno/types.js';
...
export function registerRequestTools(server: McpServer, deps: RequestToolsDeps): void {
    ...
    async (args) => {
      try {
        const input: CreateRequestInput = {
          ...
          body: args.body
            ? { type: args.body.type as BodyType, content: args.body.content, formData: args.body.formData }
            : undefined,
          auth: args.auth ? { type: args.auth.type as AuthType, config: args.auth.config } : undefined,
          query: args.query,
          folder: args.folder,
      ...
      try {
        ...
        for (let i = 0; i < args.requests.length; i++) {
          ...
          const input: CreateRequestInput = {
            ...
            url: req.url,
            headers: req.headers,
            body: req.body ? { type: req.body.type as BodyType, content: req.body.content } : undefined,
            auth: req.auth ? { type: req.auth.type as AuthType, config: req.auth.config } : undefined,
        ...
        return ok(
          `✅ Test suite "${args.suiteName}" created with ${successCount} requests${failCount > 0 ? ` (${failCount} failed)` : ''}`
        );
      } catch (error) {
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        return ok(
          `✅ CRUD operations for "${args.entityName}" created with ${successCount} requests${failCount > 0 ? ` (${failCount} failed)` : ''}`
        );
      } catch (error) {
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        const results = await requestBuilder.createAuthRequests(
          args.collectionPath,
          args.baseUrl,
          args.authType as AuthType,
          args.folder
        );
        const successCount = results.filter((r) => r.success).length;
        const failCount = results.filter((r) => !r.success).length;
        ...
        return ok(
          `✅ Auth requests created with ${successCount} requests${failCount > 0 ? ` (${failCount} failed)` : ''}`
        );
      } catch (error) {

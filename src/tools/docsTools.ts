...
export function registerDocsTools(server: McpServer, deps: DocsToolsDeps): void {
  ...
  server.registerTool(
    'set_docs',
    {
      ...
      description:
        'Write/replace markdown documentation at the collection, folder, or request level. Collection docs live in bruno.json; folder docs live in a folder.bru; request docs live in the request\'s own .bru docs block.',
      inputSchema: {
    ...
    async (args) => {
      try {
        if (args.target === 'collection') {
          if (!args.collectionPath) return err('❌ collectionPath is required when target is "collection"');
        ...
        if (args.target === 'folder') {
          if (!args.folderPath) return err('❌ folderPath is required when target is "folder"');
        ...
        if (!args.bruFilePath) return err('❌ bruFilePath is required when target is "request"');
  ...
  server.registerTool(
    'generate_docs_site',
    {
      ...
      description:
        'Render a single self-contained HTML API docs page for a collection (collection/folder/request docs, methods, URLs, headers, with client-side search). A lightweight, fully scriptable stand-in for Bruno\'s desktop-only "Generate Docs" feature.',
      inputSchema: {
